...
def index(request):
    allContact = ContactBook.objects.filter(user=request.user.id).order_by("-created")
...
def create(request):
    ...
    if contactBookForm.is_valid():
        ...
        print('mobile: ', contactBookForm.cleaned_data['mobile_number'])
        print('email: ', contactBookForm.cleaned_data['email'])
        print('label: ', contactBookForm.cleaned_data['label'])
        print('name: ', contactBookForm.cleaned_data['person_name'])
